...
int main(int argc, char* argv[])
{
  ...
  while ((opt = getopt(argc, argv, "hqcvHm:t:x:Q:")) != -1) {
    switch (opt) {
  ...
  if (argv[optind+1] != NULL) {
      ...
      fprintf(stderr, "Query file detected: %s \n", queryfile);
  }
...
//    if (!quiet) fprintf(stderr, "row %d has %d cols\n", row, col) ;
  ...
  if (!quiet) fprintf(stderr, "\rLoaded %d samples x %d allele calls\n", nrow, ncol);
    ...
    // process query file
    char** q_id  = (char**) calloc_safe( MAX_ASM, sizeof(char*) );
    char* q_buf = (char*) calloc_safe( MAX_LINE, sizeof(char) );
    uint64_t ** q_call = (uint64_t **) calloc_safe( MAX_ASM, sizeof(uint64_t *) );
    int q_row = -1;
    int q_ncol = 0;
    // read query file one line at a time
    FILE* q_in = (FILE*) fopen(queryfile, "r");
    if (! q_in) {
    fprintf(stderr, "ERROR: can not open file '%s'\n", queryfile);
    exit(EXIT_FAILURE);
    }
    while (fgets(q_buf, MAX_LINE, q_in))
    {
    // cleanup non-numerics in NON-HEADER lines
    // No need to do this when hashed chewbbaca output is used.
    // '-' and 'NA' get evaluated to 0 by hex_to_int64.
    if (!use_hashed_profiles && q_row >=0){
      cleanup_line(q_buf);
    }

    // scan for tab separated values
    char* q_save;
    char* q_s = strtok_r(q_buf, DELIMS, &q_save);
    int q_col = -1;
    while (q_s) {
      //fprintf(stderr, "DEBUG: row=%d col=%d s='%s'\n", row, col, s);
      if (q_row >= 0) {
        if (q_col < 0) {
          ...
          q_id[q_row] = strdup(q_s);
          q_call[q_row] = (uint64_t *) calloc_safe(q_ncol, sizeof(uint64_t *));
        }
        else {
          // INF-xxxx are returned as -ve numbers
          if (use_hashed_profiles) {
            q_call[q_row][q_col] = hex_to_int64(q_s) ;
          }
          else{
            q_call[q_row][q_col] = abs( atoi(q_s) );
          }
        }
      }
      else {
      ...
      q_col++;
      q_s = strtok_r(NULL, DELIMS, &q_save);
    }
    q_row++;
    //    if (!quiet) fprintf(stderr, "row %d has %d cols\n", row, col) ;
    if (!quiet) fprintf(stderr, "\rLoaded row %d", q_row);
    if (q_row==0) q_ncol = q_col;
    if (q_row >= MAX_ASM) {
      fprintf(stderr, "Too many rows, can only handle %d\n", MAX_ASM);
      exit(EXIT_FAILURE);
    }
    ...
    // what we collected for query matrix
    if (!quiet) fprintf(stderr, "\rLoaded Query %d samples x %d allele calls\n", q_nrow, q_ncol);
    
    // build an output matrix (one dimensional j*nrow+i access)
    int* dist = calloc_safe(nrow*q_nrow, sizeof(int));
    for (int j=0; j < q_nrow; j++) {
        if (!quiet) fprintf(stderr, "\rCalculating distances: %.2f%%", (j+1)*100.0/q_nrow);
        for (int i=0; i < nrow; i++) {
          int d = distance(q_call[j], call[i], ncol, maxdiff);
          dist[j*nrow+i] = d;
        }
    }
